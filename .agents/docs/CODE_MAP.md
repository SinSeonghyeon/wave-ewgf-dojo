...
후원      DONATE_URL{ko,en,ja} 외부 링크(ko 카카오페이, en/ja Ko-fi) · DONATE_QR{ko:'donate-kakao.png'}: 휴대폰 전용 링크인 언어는 #donateBtn 클릭을 가로채 #donateDlg(QR + 휴대폰용 직접 링크 #donateOpen)를 연다(modalOpen에 포함) · renderDonate(): 주소가 비면 푸터 #donate 숨김, 아니면 href·QR src. renderAll()에서 호출. 문자열 donate.*
          donate-kakao.png는 DONATE_URL.ko를 담은 QR(제작 시 1회 생성한 정적 파일). 주소가 바뀌면 다시 만든다(스모크 테스트가 스크래치 페이지 옆에 복사)
