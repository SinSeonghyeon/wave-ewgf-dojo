...
- 입력 이력(2026-09-19): historyRows는 최대 40행을 역순 1회 순회하며 다음 방향 시각을 누적해 방향 유지 시간을 구한다. 80ms 표시는 유지하되 `historyMarkup`이 같으면 DOM을 교체하지 않는다. 입력 패널은 씬 하단 힌트 바로 위까지 flex로 채우고, 목록만 내부 스크롤. 터치는 하단 46% 컨트롤·힌트 공간을 제외한다. 최신 행 우선과 방향 홀드/버튼 간격 계산은 그대로다.
...
- `BGM_TRACKS`는 `bgm/` 폴더의 경로 목록. `tools/update-bgm.js`가 로컬 fallback을 갱신하고 배포에서는 Jekyll `site.static_files`로 만든 `bgm/playlist.json`을 우선 로드한다. HTTP에서는 목록 응답 전 재생을 보류하고 4초 타임아웃/네트워크·JSON 오류면 fallback을 쓴다. `bgmPick`은 직전 index를 제외해 균등 선택, `store.bgmLast`는 재접속 때 제외할 파일 경로(순서 변경에 안전). `snd.track`/`snd.paused`는 현재 곡/명시적 일시정지. Audio.loop=false, onended→bgmSelect→bgmSync. `bgmNext`는 mute/pause 보존. 상단 bgm-player에는 bgmBtn/bgmPlay/bgmNext/bgmTrack. 부트 때 Audio 생성 없음, Web Locks 소유·가시성·마스터 음소거 조건 유지.
