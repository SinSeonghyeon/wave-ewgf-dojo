...
- [x] 2-3-1. 후원 유도 개선 (2026-09-15, 사용자 요청). 개인 최고 기록 결과창에 KST 하루 1회만 `1,000원부터 + 도메인·서버 유지/기능 개발` 안내를 보이고 밥 농담은 작은 글씨로 유지. 실제 입력이 이어지는 연습 시간 10분(30초 무입력 시 멈춤) 뒤 헤더 버튼을 8초 금빛 강조하고 `응애… 밥 줘… 🍚` 말풍선을 KST 하루 1회 표시. 측정·대화상자·보상 연출 뒤로 미루고, 동작 줄이기/연출 끄기에서는 정적. `store.donateResultDay/donateNudgeDay/donatePlayDay/donatePlayMs`로 새로고침 복원. 세 언어 적용, 판정·순위·워커 변경 없음.
- 2026-09-15: 2-3-1 코드 리뷰 P2 3건 반영. 같은 결과창 중복 열기에서 후원 안내가 사라지지 않게 하고, 활성 말풍선 중 측정·대화상자·보상 연출이 시작되면 당일 소모 없이 재표시하며, pagehide에서 30초 체크포인트 사이 연습 시간도 저장하도록 수정.
...
- 2026-09-15: 2-3-1 완료(후원 유도 개선 + 리뷰 P2 3건). 개인 최고 결과창 일 1회 안내, 구체적인 소액·사용처 본문 + 작은 밥 농담, 실제 연습 10분 뒤 헤더 금빛/말풍선 일 1회와 저장 복원을 추가. 중복 결과 열기·차단 UI 중 활성 말풍선 연기·pagehide 미저장 시간 회귀 테스트 포함 단위 95개와 전체 브라우저 스모크 통과(JS 오류 0).
