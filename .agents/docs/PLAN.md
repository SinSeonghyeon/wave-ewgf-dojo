...
- [x] 1-9. 한마디 게시판 (2026-09-12 사용자 결정: 실시간 채팅 대신). 닉네임 + 200자 한 줄, 최신 50개, 1분마다 새로 고침, IP당 1분 3개(Cloudflare 레이트 리밋 바인딩), 관리자 삭제는 `ADMIN_TOKEN`으로 `DELETE /posts/:id`. 수정·신고 없음. 스팸이 문제 되면 그때 검토.
  ...
  - [x] 대댓글 (2026-09-14 사용자 요청): 원글마다 여러 개의 1단계 대댓글, 오래된 순 표시. 원글과 같은 닉+토큰·200자·`POST_LIMIT` 사용. 워커 `POST /reply`, `replies` 테이블, 원글 삭제 시 함께 삭제. **스키마 적용·워커 재배포는 사용자 작업**(사이트 푸시보다 먼저).
...
- 2026-09-15: 대댓글 코드 리뷰 P2 3건 반영. 앱의 원글·투표·답글 쓰기를 공통 busy 가드로 직렬화하고 `postsSeq`로 먼저 시작한 목록 조회의 낡은 응답을 폐기해 전체 rows 스냅샷 덮어쓰기를 방지. 워커 `/posts`는 첫 SELECT에서 조회한 원글 ID 집합으로만 답글을 가져오며, `/reply`는 부모 확인+삽입을 원자적 `INSERT … SELECT … WHERE EXISTS`로 실행해 관리자 삭제 경합의 고아 행을 차단. 세 경합 회귀 테스트 추가(단위 97개 통과).

- 2026-09-14: 한마디 대댓글(사용자 요청, 결정 20). 앱: 원글마다 `답글 N` 버튼·인라인 입력창·시간순 중첩 목록, ko/en/ja 문구, 작성 중/실패/삭제·닉네임 게이트 처리. 워커: `replies(id,post_id,nick,text,created_at)` + `POST /reply`, `/posts` 응답의 `replies[]`, 원글 삭제 시 연쇄 정리. 원글과 대댓글은 같은 `POST_LIMIT` 버킷(IP당 합계 1분 3개)을 공유한다. 단위 테스트에 다중 대댓글·검증·정렬·레이트 리밋·삭제와 프런트 제출/재시도/언어 전환을 추가하고 스모크에 실제 작성·D1 확인을 추가. **워커 스키마 적용·재배포는 사용자 작업이며 사이트보다 먼저**.
