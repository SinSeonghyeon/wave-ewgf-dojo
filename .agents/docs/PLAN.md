...
- [x] 4-12. 동작별 보조 키 할당 (2026-09-15). 방향·버튼마다 기존 기본 키 외 보조 키 1개를 `store.altKeys`에 저장. 두 키 모두 같은 입력 경로를 타며, 방향은 둘을 같이 누른 뒤 하나만 떼도 유지되고 버튼은 둘을 같이 눌러도 논리 입력 1회만 발생한다. 설정에는 기본/보조 두 칸과 보조 키 제거(Delete/Backspace)를 표시하고, 전체 키 중복·방향키 충돌은 기존처럼 거부. 옛 `store.keys` 저장값 호환 유지.

## 진행 로그

- 2026-09-15: 동작별 보조 키 1개 할당 완료 및 리뷰 P2 반영. 설정의 8개 동작마다 기본/보조 슬롯, `altKeys` 로드 검증·저장, 방향·버튼 합산 입력, 보조 제거 안내를 ko/en/ja로 추가. 같은 버튼의 두 키를 동시에 누르면 두 번째 `keydown`을 억제해 초풍 통계·측정 횟수를 한 번만 소비하도록 수정하고 양방향 순서 회귀 테스트를 추가. 단위 94개·전체 Chrome 스모크 통과(JS 오류 0).
