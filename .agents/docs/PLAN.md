...
- [x] 1-10. 공지사항·패치 노트 (2026-09-14, 자동 표시 2026-09-15). 헤더 종 버튼 → 팝업, 최신 공지 미열람 시 `NEW` 배지. 첫 방문은 배지만, 기존 방문자는 새 공지를 접속 후 1회 자동 표시(측정·다른 팝업 중에는 대기). 공지는 `index.html`의 `NOTICES`와 ko/en/ja I18N 키로 관리하며 최신 ID를 `store.noticeSeen`에 저장한다. 백엔드 없음.
...
- [x] 4-4. 모바일 터치 컨트롤 (2026-09-12 완료. 아래 원형 패드 설계는 당시 기록이며, 2026-09-15에 4-12의 ←·↓·→ 버튼 방식으로 교체). 설계 2026-09-12, 사용자 요청: "터치패드가 게임 뷰에 출력, 좌측 방향키 + 우측 1234 2×2, 모바일에서만". 사용자 확인 완료(스테이지 안 오버레이 · 순위 동일 처리) → 구현 진행.
...
- [x] 4-11. 헤더 배경음 토글 (2026-09-14). 언어 선택기 왼쪽 인라인 SVG 스피커(버튼 45px·아이콘 27px, 기존 대비 1.5배)와 설정 창 배경음 토글을 `store.bgm`으로 동기화. 효과음 유지·새로고침 복원·세 언어 툴팁.

- [x] 4-12. 모바일 방향 버튼 개선 (2026-09-15). 원형 슬라이드 패드를 독립 ←·↓·→ 버튼으로 교체하고 ↓+좌/우 멀티터치로 대각선을 만든다. 설정에서 방향 버튼 크기(70~140%)·가로/세로 위치(0~100%)를 조절하고 브라우저에 저장한다. 기본 100%는 320px·390px에서도 공격 버튼을 침범하지 않으며, 사용자 지정 확대는 선택값을 유지해 좁은 화면에서 겹칠 수 있다. 공격 버튼과 기존 판정 경로는 유지.

## 진행 로그

- 2026-09-15: 여러 기능 브랜치의 일괄 합류를 위한 인수인계 구조 추가. 영구 절차는 `.agents/docs/MERGE_PROCESS.md`, 브랜치별 임시 문서는 `.agents/handoffs/<브랜치>.md`에 두고 공지 ko/en/ja 초안·배포 순서·충돌 지점·검증 결과를 전달한다. 통합 담당자가 모두 반영한 뒤 임시 문서는 `main` 합류 전에 삭제한다.

- 2026-09-15: 사용자 요청으로 첫 공지의 표시 날짜·읽음 ID·I18N 키를 2026-09-15 기준으로 통일했다. 단위·워커 테스트 96개 통과. Chrome 스모크는 9월 15일 공지·재접속 검사를 통과해 후반 보상 시나리오까지 진행했으나 `Input.dispatchKeyEvent`의 기존 CDP 15초 타임아웃으로 중단됐다.

- 2026-09-15: 공지 코드 리뷰 2건 반영. 브라우저 스모크의 읽음 ID 기대값을 원본 `NOTICES` 첫 항목에서 파생해 새 공지 누적 시 함께 바뀌게 하고, 날짜 전용 공지 날짜는 `Intl.DateTimeFormat`의 UTC 시간대로 고정해 UTC+12 이상에서도 다음 날로 밀리지 않게 했다. 단위·워커 테스트 96개 통과. 전체 Chrome 스모크는 공지·재접속 검사를 2회 통과해 후반 시나리오까지 진행했으나, 각각 보상 연출 조회와 화면 스냅샷 조회에서 기존 CDP 15초 타임아웃으로 중단됐다(JS 오류·검사 실패 보고 전 단계의 브라우저 응답 중단).

- 2026-09-15: 공지 자동 표시(사용자 요청). 저장소가 이미 있던 기존 방문자에게 미열람 최신 공지를 접속 0.5초 뒤 한 번 자동으로 열고, 첫 방문은 닉네임 창과 연달아 뜨지 않게 `NEW` 배지만 표시. 측정·카운트다운·결과·다른 대화상자·보상 연출 중이면 0.75초 간격으로 기다리며 기존 흐름을 취소하지 않는다. 단위·워커 테스트 95개 통과. 자동 공지 전용 실제 Chrome 검사(첫 방문/기존 미열람/읽은 뒤 재접속) 통과. 전체 `smoke-chrome`은 서로 다른 CDP 명령에서 15초 타임아웃 4회, 제한을 임시 30초로 늘려도 입력 명령 타임아웃 1회로 완주하지 못했으며 임시 변경은 원복함(기능 단정 실패·JS 오류 보고 전 단계의 브라우저 응답 중단).

- 2026-09-14: 1-10 완료(공지사항·패치 노트). `feature/notices` 브랜치에서 헤더 종 버튼, 공지 모달, 브라우저별 읽음 ID와 `NEW` 배지, ko/en/ja 렌더링을 추가. 새 패치는 `NOTICES` 맨 앞과 세 언어 키를 함께 추가하면 최신 ID 변경으로 배지가 다시 켜진다. 단위·워커 테스트 94개와 전체 브라우저 스모크 통과(JS 오류 0), 390×844 실화면에서 가로 넘침 없이 버튼·팝업 배치 확인. 워커 변경 없음.

- 2026-09-15: 터치 크기 리뷰 P2를 사용자 결정에 맞게 반영. 기본 100%만 320px·390px에서 공격 버튼과 비침범을 보장하고, 사용자가 키운 크기는 가용 폭으로 강제 축소하지 않는다. 320px 기본 비침범과 140% 선택값 유지 회귀 검사를 스모크에 추가. 단위 93개·별도 모바일 스모크(실제 터치 초풍 포함) 통과. 전체 스모크는 기존 CDP `Runtime.evaluate` 타임아웃으로 터치 구간 전에 중단.

- 2026-09-15: 4-12 완료(모바일 방향 버튼 개선). `#tpad` 원형 벡터 입력을 포인터별 `#tdirs` 3버튼과 `touchKeys` 조합 입력으로 교체, `store.touchSize/touchX/touchY` 검증·슬라이더·반응형 제한 배치 추가, ko/en/ja·README·CODE_MAP·설계 결정 11 갱신. 단위 93개 통과. 모바일 전용 Chrome 스모크에서 실제 →,N,↓,↘+2 초풍·설정 즉시 반영/저장·JS 오류 0 확인. 전체 스모크는 기존 결과/게시판 단계 CDP 응답 타임아웃으로 3회 중단(터치 구간 전, 기능 오류 출력 없음).

- 2026-09-14: 사용자 요청으로 헤더 스피커를 언어 선택기 왼쪽으로 이동하고 버튼 30→45px·아이콘 18→27px(1.5배)로 확대. 단위 93개·브라우저 스모크 통과(JS 오류 0), 1366/390px 배치 확인.

- 2026-09-14: 4-11 완료(헤더 배경음 토글). `store.bgm` 검증·저장, `setBgm`으로 헤더/설정 동기화, 배경음 켜기 시 마스터 복원·효과음 볼륨 유지. `bgmSync` 조건과 슬라이더 비활성 반영, 인라인 SVG·세 언어 툴팁 추가. 단위 93개·전체 브라우저 스모크(JS 오류 0)·기존 소리 스모크 통과, 1366/390px 캡처 배치 확인. 워커 변경 없음.
