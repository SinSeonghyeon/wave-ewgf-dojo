...
- [x] 1-10. 본인 순위 기록 삭제 (2026-09-14 사용자 요청): 본인 행에만 삭제 버튼·확인창, `DELETE /score {board,nick,token}`이 소유권을 검증해 현재 보드의 본인 행만 삭제. 성공 응답으로 순위·참가 수·`cut10` 즉시 갱신. 워커 재배포는 **사용자 작업**(사이트 푸시보다 먼저).
...
- [x] 2026-09-17: 독립 모델용 영상 제작 인계 패키지 작성 — `.agents/docs/wsc-handoff/`에 최신 브리프·통합 대본·근거별 데이터/16개 더미 슬롯·원본 소재 경로·플랜/QA·복사용 프롬프트·내부 참고를 분리. 캐릭터는 내레이터만, RP 7→8→9, 8+1 추천 삭제, 대안 3종 비교 반영. 과거 시안 제외, 비교 미측정 값은 null. 최초 ZIP은 `.sandbox/wsc-handoff-delivery/`에 보존. 사용자 요청으로 최신 문서와 원본 캐릭터·메모를 담은 외부 인계 ZIP을 `C:\Users\gus54\Videos\WSC-Production-Handoff\2026-09-17\wsc-production-handoff.zip`에 별도 보관하고 브랜치 `SinSeonghyeon/workspace02`의 인계 문서에 경로를 기록. 세션별 복사 사용, 원본 플레이는 별도 경로.

- [x] 2026-09-17: 웨캔기어 캐릭터 연출 샘플 08 — 사용자 지정 이미지의 수염 캐릭터를 벡터로 재작화, 실패 반응·기록지·프레임 칸 이동 + 실제 플레이 전체화면/축소 비교 + 임시 TTS. 28초 MP4, `.sandbox/wsc-video/review-character-08.html`. 이전 본편/쇼츠·원본 보존. 전체 영상 반영 전 사용자 캐릭터/연출 피드백 대기.

- [x] 2026-09-16: 웨캔기어 실제 촬영본 1차 편집 — 원본에서 기상어퍼/풍신권/선 자세 기술 장면 선정, 실패 비교·입력 기록 확대·기존 프레임 도식·임시 한국어 TTS 결합. 본편 95.3초·쇼츠 47초 MP4, `.sandbox/wsc-video/review-edit-07.html`. 원본 보존·미디어 비커밋. 실패 원인과 각 촬영 조건의 최종 대조는 남음.

- [x] 2026-09-16: 웨캔기어 시안 06 — 성공 입력 분석 앞에 풍신권/선 자세 기술 실패 비교와 직접 프레임별 실험한 동기 추가. 본편 116초·쇼츠 58초. `.sandbox/wsc-video/preview-v6.html`. 실패별 원인 설명은 정확한 입력 조건 확인 대기.

- [x] 2026-09-16: 웨캔기어 시안 05 — 타이틀 삭제, 같은 커맨드 문구, 화살표·★ 표기 통일, 선행 입력과 프레임축 통합, 뒤 7·8·9f 구간 → RP 허용 폭으로 설명 분리, 연습 목표 8+1. `.sandbox/wsc-video/preview-v5.html`. 다음은 실제 촬영본 삽입.

- [x] 2026-09-16: 웨캔기어 시안 04 — 상단 가제·번호·하단 제작 표기 제거, 설명 카드 대신 비대칭 커맨드와 그려지는 밑줄·원, 절제된 질감과 컷 전환. `.sandbox/wsc-video/preview-v4.html`. 기존 시안 보존.

- [x] 2026-09-16: 웨캔기어 모션 시안 03 — A/B 전체 화면에서 비교 위치로 축소 이동, 커맨드 칸·RP 이동·타임라인 확대, 본편 90초로 압축. `.sandbox/wsc-video/preview-v3.html`, 설계 `VIDEO_WSC.md`.

- [x] 2026-09-16: 웨캔기어 영상 시안 02 — 기술 정의·3단계 입력 설명을 추가하고 도입 단축. 다국어 자막/음성은 제작 설계에 분리 계획 기록.

- [x] 2026-09-16: 웨캔기어 영상 구성 시안(본편 150초·쇼츠 45초, 플레이 자리표시자·프레임 애니메이션) 작성. 내부 참고 링크·촬영 검증 항목은 `VIDEO_WSC.md`, 비커밋 미리보기는 `.sandbox/wsc-video/preview.html`. 실제 영상·TTS·최종 출력은 후속 작업.
...
- 2026-09-14: 본인 순위 기록 삭제 완료. 본인 행에만 3개 언어 삭제 버튼·확인창을 표시하고, 워커가 닉네임+토큰으로 소유자를 확인해 현재 보드의 해당 행만 삭제하도록 했다. 다른 사람 토큰·다른 보드 보존·반복 삭제·실제 Chrome UI→D1 흐름을 테스트했다. **사이트 푸시 전 워커 재배포 필요**.

- 2026-09-15: 본인 기록 삭제 리뷰 P2 2건 반영. 삭제 중 닉네임 변경의 순위 재조회를 완료 뒤 예약하고, 응답의 `me` 닉이 현재 닉과 다르면 즉시 본인 표시·삭제 버튼에서 제외한다. `/submit` 진행 중 삭제를 막고 삭제 중 새 등록은 완료 뒤 실행해 두 점수 쓰기를 직렬화했다. 경합 회귀 테스트 2개 추가(단위 97개 통과).
