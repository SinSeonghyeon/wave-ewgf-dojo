...
- [x] 4-12. 웨캔기어 전용 3단계 연습 (2026-09-18). 확정 A=8/9/10f → B=1/1~2/1~3f, 통합 타임라인·독립 세션 기록·세 언어·1P/2P·모바일. 초풍은 공통 60Hz 동일 슬롯 판정으로 변경, 기존 웨이브 623 허용 확인. 서버 변경 없음. 사용자 직접 조작·게임 리듬 대조 대기.

- [x] 4-13. 웨캔기어 UI 간소화·도전·연출 (2026-09-18 후속 요청). 6N23 묶음/3 이후 타임라인, 단계 버튼 제거·우측 A/B 평가, 로컬 20회 도전(제안 기본안), 기상어퍼 동작·팝·일반 더미 반응. 기존 기능과 서버 분리 유지.

- [x] 4-14. 웨캔기어 도전 최종 확정 (2026-09-18): 10개 랜덤 과제, N=0~5 웨이브 후 정확히 N+1번째에서 성공. 우측 요구/현재 횟수와 결과 목표/실제 횟수 표시. 유효 연결만 연속 합산, 중단은 같은 과제 재시도. 기존 20회 기본안을 대체.

- [x] 4-15. 공통 커맨드·프레임 표시 (2026-09-18): 모든 모드 웨캔기어, wsc에서도 초풍·나락·통발 사용. 인식/연출은 공유하고 WSC 및 기존 통계 집계 분리 유지. 하단 입력 이력 복구, 현재 칸 강조·실시간 A/B 숫자·모바일 진행 스크롤. 랜덤 과제 N=0~3으로 축소.

- [x] 4-16. 기술별 효과음·피격 효과 (2026-09-18): 사용자 제공 기상어퍼/나락/통발/피격음 4개 연결. 더미 명중 후 피격 반응 시작에 1회 재생·180ms 충격 원/방사선/불꽃, fx/reduced와 음소거/볼륨 분리. 도전 횟수만 실패한 기술 성공도 기술음 재생. 음원 4개는 직접 제작 또는 공개 사용 권한이 있다고 사용자 확인 완료. 커밋·푸시 없음.

- [x] 4-17. 피격음 타이밍·기술음 볼륨 조정 (2026-09-18): 통발/기상어퍼 음량 80%, 설정 변경 후 유지. 공통 110ms 피격 예약을 기술별 160~200ms로 늦추고 피격음·충격 효과·더미 반응을 동기화. 게임 판정·점수 변경 없음.

- [x] 4-18. 피격음 내부 지연 보정 (2026-09-18): 직접 청취 검증이 아니었음을 정정. FFmpeg·Chrome 디코딩으로 강한 피격음 시작180ms 확인, SFX_START.hit=0.18로 앞부분 생략·모든 효과음 선로딩. 원본/gain/접촉 시간 유지. 브라우저 소리 포함 전후 비교 영상 생성.

- [x] 4-19. 정리 음원 교체·백대시 효과음 (2026-09-18): 사용자 제공 5개 원본 복사, 백대시 발동에만 전용음 추가. 새 피격음 onset120ms로 보정 재측정, 통발/기상어퍼80%·contact 시간 유지.

- [x] 4-20. 초풍 안내 리뷰 수정·병합 인계 갱신 (2026-09-18): ko/en/ja 초풍20회 힌트·늦은 성공 코치를 공통60Hz 동일 프레임 기준으로 수정. `.agents/handoffs/SinSeonghyeon-workspace02.md`에 미커밋 기능 범위·최신 main 보존 항목·배포 순서·공지 초안 기록. 미사용 판정 폭 번역 키 12개는 사용자 승인으로 삭제 완료.
