...
4. 단일 파일, 외부 라이브러리 없음. 차트는 직접 SVG 문자열. 허용 외부 자원은 Google Fonts뿐.
...
- 코드 변경 후 최소 검증: 단위 테스트 → 브라우저 스모크 테스트 → 커밋·푸시 → Pages 반영 확인(약 1분, `curl -s <배포 주소> | grep -o "<title>[^<]*</title>"`). 실패한 테스트는 결과 그대로 보고한다.
...
- 같은 폴더에서 여러 에이전트 세션이 동시에 돌 수 있다. 시작할 때 `git status`, `git log origin/main..main`, `git log main..origin/main`으로 갈라짐을 확인한다. 로컬 main이 원격과 다르면 작업 트리 변경이 없는지 확인한 뒤 `git fetch && git reset --hard origin/main`으로 맞춘다.
- 커밋하지 않을 임시 파일·실험 스크립트는 `.sandbox/<topic>/`(gitignore됨)에 둔다. 영속할 문서만 `.agents/docs/`에 둔다.
- 일반 커밋과 `git push`는 테스트 통과 후 에이전트가 직접 한다. force push, 히스토리 재작성, 공개 저장소 생성, 저장소 설정·권한 변경은 사용자가 그 작업을 명시적으로 지시했을 때만 한다. 도구 권한이 막히면 필요한 명령을 안내하고 사용자가 `! <명령>`으로 실행하게 한다.
