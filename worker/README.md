...
| `GET /scores?board=&week=` | 관리자. 그 주(기본 이번 주) 보드의 **모든** 행을 id·차단 표시(`banned`)와 함께 |
...
- 비밀 `ADMIN_TOKEN`(관리자 경로 전용: 게시글·기록 삭제, 섀도 밴, 주간 전체 기록 조회)은 `npx wrangler secret put ADMIN_TOKEN`으로 넣는다. 값은 저장소에 두지 않는다(로컬 `.sandbox/admin-token.txt`, gitignore).
...
```powershell
node tools/board-admin.js top wave10          # 이번 주 wave10 전체 행: 순위(차단 행은 BAN)·id·닉·점수·detail·시각. 뒤에 날짜(YYYY-MM-DD)를 주면 그 날이 든 주
